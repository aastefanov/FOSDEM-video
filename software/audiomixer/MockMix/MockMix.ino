...
// not real printf, but the avr boards don't support it :(

char xprintf_buff[128];
void xprintf(char* format, ...) {
	va_list args;
	va_start(args, format);
	vsprintf(xprintf_buff, format, args);
	va_end(args);
	Serial.write(xprintf_buff);
}
	...
	switch (cmd) {
		case 'l': {
			...
			if (channel > 5 || channel < 0) {
				xprintf("usage: l<channel> in range 1-6\r\n");
				return;
			}
			float gains[] = {
		...
		case 'h': {
			xprintf("Audio mixer commands:\r\n");
			xprintf("e<0|1>            set the UART echo state\r\n");
			xprintf("s<in><out><gain>  set the gain for one crosspoint\r\n");
			xprintf("l<out>            print the levels sent to a specific output\r\n");
			xprintf("[lf]              print the whole human-readable mixing matrix\r\n");
		}
		case 13:
			// Special case, print human readable status
			xprintf("      IN1  IN2  IN3  PC   USB1 USB2\r\n");
